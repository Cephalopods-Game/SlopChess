...
export default class Board {
	...
	getPawnMoves(piece, location) {
		...
		if (
			move &&
			((this.getPiece(move) && this.getPieceOnLocationColor(move) !== color) || move === this.configuration.enPassant)
		) {
			moves.push(move);
		}
	...
	getQueenMoves(piece, location) {
		const moves = [...this.getRookMoves(piece, location), ...this.getBishopMoves(piece, location)];
		return moves;
	}
			...
			moves[from].map((to) => {
				...
				scoreTable.push({
					...
					score:
						...
						Math.floor(Math.random() * (this.configuration.halfMove > 10 ? this.configuration.halfMove - 10 : 1) * 10) /
							10,
				});
			});
		}
	...
	testMoveScores(playingPlayerColor, level, capture, initialScore, move, depth = 1) {
		...
		if (this.configuration.isFinished) {
			return {
				score:
					this.calculateScore(playingPlayerColor) + (this.getPlayingColor() === playingPlayerColor ? depth : -depth),
				max: true,
			};
		}
